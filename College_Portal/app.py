...
import psycopg2
from psycopg2 import errors as pg_errors  # Import psycopg2 errors for specific handling
...
def get_db():
    if 'db' not in g:
        try:
            ...
            # Do not use flash here, as there's no active request context during init_db()
            print(f"Database connection error: {e}")
            g.db = None
            # Re-raise to prevent app from starting without DB connection if crucial
...
def init_db():
    # We ensure we are within an app.app_context() when this is called externally
    conn = None
    ...
    try:
        ...
        # Create Users table (modified to include status column)
        cur.execute("""
            CREATE TABLE IF NOT EXISTS users (
                ...
                status VARCHAR(20) DEFAULT 'active' CHECK (status IN ('active', 'suspended', 'expelled')), -- Added status column
                created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
            );
        """)
        conn.commit()
        print("Users table checked/created/updated with status column.")
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS housing_applications (
                id SERIAL PRIMARY KEY,
                user_id INTEGER REFERENCES users(id) ON DELETE SET NULL, -- Nullable for unauthenticated users
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS logs (
                ...
                action_type VARCHAR(100) NOT NULL, -- e.g., 'student_registration', 'course_add', 'student_suspend'
        ...
        cur.execute("CREATE INDEX IF NOT EXISTS idx_logs_timestamp ON logs (timestamp);")  # Index for logs
        conn.commit()
            ...
            try:
                ...
                for data in faculty_data:
                    users_to_insert.append((
                        data[0], generate_password_hash(data[1]), data[2], 'faculty', data[3], data[4], 'active'
                        # Added 'active' status
                    ))
                ...
                cur.executemany(
                    "INSERT INTO users (username, password_hash, email, role, first_name, last_name, status) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                    users_to_insert
                )
                conn.commit()
...
def load_logged_in_user():
    ...
    if user_id is not None:
        ...
        if conn:
            cur = conn.cursor()
            try:
                # Fetch status along with other user details
                cur.execute(
                    "SELECT id, username, password_hash, email, role, first_name, last_name, status FROM users WHERE username = %s",
                    (user_id,))
                ...
                if user_data:
                    g.user = {
                        ...
                        "status": user_data[7],  # Added status
                        "name": f"{user_data[5]} {user_data[6]}"
                    }
                    # If user is suspended or expelled, log them out
    ...
    if request.method == 'POST':
        ...
        if user_data:
            if check_password_hash(user_data[1], password):
                user_status = user_data[5]  # Get status
...
def register():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            cur.execute(
                "INSERT INTO users (username, password_hash, email, role, first_name, last_name, status) VALUES (%s, %s, %s, %s, %s, %s, 'active') RETURNING id",
                (username, hashed_password, email, role, first_name, last_name)
            )
            new_user_id = cur.fetchone()[0]
            conn.commit()

            # Log the registration
            log_action(new_user_id, 'user_registration', f"New {role} account registered: {username}")
        ...
        try:
            ...
            for ann in raw_announcements:
                announcements.append({
                    "title": ann[0],
                    "content": ann[1],
                    "date": ann[2].strftime('%B %d, %Y')  # Format date nicely
                })
        except psycopg2.Error as e:
...
def student_portal():
    ...
    if conn and g.user:
        cur = conn.cursor()
        try:
            # Fetch courses the logged-in student is enrolled in, along with their grades
            cur.execute("""
                SELECT
                    c.course_code,
                    c.title,
            ...
            for course in raw_courses:
                ...
                enrolled_courses.append({
                    ...
                    "grade_value": course[6] if course[6] else "N/A",  # Display grade value
                    "score": course[7] if course[7] is not None else "N/A"  # Display numeric score
                })
...
def manage_events():
    ...
    if request.method == 'POST':
        ...
        try:
            if action == 'add':
                ...
                if event_id:
                    # Fetch title before deleting for logging
    ...
    # GET request: Display current events and potentially pre-fill for editing
    events = []
    cur = conn.cursor()
    try:
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                credits = None  # Handle invalid credit input

            if action == 'add':
                if course_code and title and credits is not None and credits > 0:
                    try:
                        cur.execute(
                            "INSERT INTO courses (course_code, title, description, credits, faculty_id) VALUES (%s, %s, %s, %s, %s)",
                            (course_code, title, description, credits, faculty_id if faculty_id else None)
                        ...
                        conn.commit()
                        log_action(g.user['id'], 'course_add', f"Added new course: {course_code} - {title}")
                    ...
                    flash('Course Code, Title, and valid Credits are required to add a course.', 'danger')
            elif action == 'update':
                if course_id and course_code and title and credits is not None and credits > 0:
                    try:
                        cur.execute(
                            "UPDATE courses SET course_code = %s, title = %s, description = %s, credits = %s, faculty_id = %s WHERE id = %s",
                            (course_code, title, description, credits, faculty_id if faculty_id else None, course_id)
                        )
                        conn.commit()
                        log_action(g.user['id'], 'course_update',
                                   f"Updated course ID {course_id}: {course_code} - {title}")
                    ...
                    flash('Course ID, Code, Title, and valid Credits are required to update a course.', 'danger')
            elif action == 'delete':
                if course_id:
                    try:
                        # Fetch course code before deleting for logging
            ...
            return redirect(url_for('manage_courses'))  # Changed redirect to stay on manage_courses page

        # For GET request: Fetch all courses for display
        cur.execute("""
            SELECT
                c.id, c.course_code, c.title, c.description, c.credits, u.first_name, u.last_name, u.id as faculty_user_id
            FROM
                courses c
            LEFT JOIN
        ...
        for course in raw_courses:
            ...
            courses.append({
                ...
                "credits": course[4],
                "faculty_name": faculty_name,
                "faculty_id": course[7]  # Pass faculty's user ID for pre-selection in edit form
            })

        # Check for edit_id query parameter
        edit_id = request.args.get('edit_id', type=int)
        if edit_id:
            cur.execute("""
                SELECT
                    id, course_code, title, description, credits, faculty_id
                FROM
                    courses
                WHERE id = %s
            """, (edit_id,))
...
def view_students():
    ...
    if request.method == 'POST':
        ...
        try:
            # Fetch student's name for logging before action
    ...
    try:
        cur.execute(
            "SELECT id, username, first_name, last_name, email, status FROM users WHERE role = 'student' ORDER BY last_name, first_name;")
...
def manage_student_courses():
    ...
    try:
        ...
        cur.execute(
            "SELECT id, first_name, last_name FROM users WHERE role = 'student' AND status = 'active' ORDER BY last_name, first_name;")
        raw_students = cur.fetchall()
        students = [{"id": s[0], "name": f"{s[1]} {s[2]}"} for s in raw_students]
        ...
        if request.method == 'POST':
            ...
            # Fetch student and course details for logging
            ...
            # After action, redirect back to the same student's view if possible
            return redirect(url_for('manage_student_courses', student_id=student_id))

        # For GET request: If a student is selected, fetch their enrollments and grades
...
def assign_grades():
    ...
    try:
        ...
        # For GET request: If a student is selected, fetch their enrollments for grading
        if selected_student_id:
            cur.execute("""
                SELECT
                    e.id as enrollment_id,
...
# Routes for general information pages
        ...
        try:
            ...
            if raw_member:
                ...
                detailed_faculty_data = {
                    ...
                    'faculty11@college.edu': {
                        "department": "Art & Design", "position": "Assistant Professor",
...
# New routes for campus life sub-pages
